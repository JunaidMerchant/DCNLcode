...
server <- function(input, output) {
  ...
  output$instruction1<- renderText(c("Please upload a .csv file with subject id, age, gender, and item-level raw data for the SRS-2 with the following, case-sensitive labels: ID, Age, Gender, Q1 through Q65 (see example below)."))
  output$instruction2<- renderText(c("In addition to the raw data, the output file will include the following columns: srs_socawr_raw_p1 srs_soccog_raw_p1 srs_soccomm_raw_p1 srs_socmot_raw_p1 srs_autman_raw_p1 srs_tot_raw_p1 srs_sci_raw_p1 srs_rrb_raw_p1 srs_socawr_t_p1 srs_soccog_t_p1 srs_soccomm_t_p1 srs_socmot_t_p1 srs_autman_t_p1 srs_tot_t_p1 srs_sci_t_p1 srs_rrb_t_p1 TscoreM_Awareness TscoreM_Cognition TscoreM_Communication TscoreM_Motivation TscoreM_Mannerisms TscoreM_TotalScore TscoreM_SCI TscoreM_RRB TscoreF_Awareness TscoreF_Cognition TscoreF_Communication TscoreF_Motivation TscoreF_Mannerisms TscoreF_TotalScore TscoreF_SCI TscoreF_RRB."))
  output$instruction3<- renderText(c("NOTE: This is based on the SRS-2 parent report, and only works for children between 8 to 13 years old. It is intended for the RDOC study that is in collaboration with DCNL at Georgetown and Children's National Autism group. This was designed to take data from Qualtrics SRS scale, score, then upload to REDCap. Please send your questions to Junaid Merchant: merchantjs@gmail.com"))
  
  
  ID=c("subject1","subject2","subject3"); Age=c(8,10,13); Gender=c("Male", "Female", "Male"); Q1=c("NOT TRUE","SOMETIMES TRUE","OFTEN TRUE"); etc=c("...","...","..."); Q65=c("OFTEN TRUE","SOMETIMES TRUE","NOT TRUE"); 
  
  disp_data=data.frame(ID,Age,Gender,Q1,etc,Q65)
