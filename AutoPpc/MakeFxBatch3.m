...
addpath(genpath('/media/jm3080/Naider/RDOC/bx'));
...
for f = 1:length(Funcs)
    ...
    if length(CurFDirs)==1
        ...
        %
        % Run Bx script on file
        if f==1
            NBackBx(Vec{1,1});
        elseif f==2
            SocStroopBx(Vec{1,1});
        elseif f==3
            Vec=findfiles(fullfile('/media/jm3080/Naider/RDOC/bx/',Funcs{1,f}),strcat('*',SubID(6:13),'*.txt'));
            StopBx(Vec{1,1});
            Vec=findfiles(fullfile('/media/jm3080/Naider/RDOC/bx/',Funcs{1,f}),strcat('*',SubID(6:13),'*.mat'));
        end
        %
